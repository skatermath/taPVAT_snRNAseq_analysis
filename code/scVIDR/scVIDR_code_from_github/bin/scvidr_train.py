...
for subparser in [parser_single, parser_multi]: 
    ...
    subparser.add_argument('--max_epochs', help='Maximum number of training epochs', default=100, type=int)
    subparser.add_argument('--seed', help='Random seed for reproducibility (default: None = unseeded)', default=None, type=int)
...
# Seed all RNGs for reproducibility. scvi.settings.seed seeds numpy, torch, and
# lightning; we also set numpy/torch directly as a belt-and-suspenders measure.
if SEED is not None:
    import scvi
...
VIDR.setup_anndata(train_adata, batch_key=DOSE_CATEGORICAL_COLUMN, labels_key=CELLTYPE_COLUMN)
